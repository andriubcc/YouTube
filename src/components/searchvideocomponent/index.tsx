...
function SearchedVideoComponent({openMenu}: IProps ) {
  // eslint-disable-next-line    
  const { user, searchedVideos } = useContext(UserContext);
  ...
  return (
    <Container openMenu={openMenu}>
    {searchedVideos.map(( video: any, index: number) => (
      <div key={index}>
         <ImageBanner  src={video.URL}/>
         <TitleContainer >
           <ChannelImage>{video.userName && video.userName[0] && video.userName[0].toUpperCase()}</ChannelImage>
           <TextContainer>
